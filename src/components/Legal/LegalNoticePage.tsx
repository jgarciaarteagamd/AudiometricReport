...
const LegalNoticePage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="bg-white text-slate-800 rounded-3xl shadow-xl p-8 sm:p-12 max-w-4xl mx-auto animate-fadeIn border border-slate-100">
      <div className="flex justify-between items-center mb-10 border-b border-slate-50 pb-6">
        <h1 className="text-3xl font-black text-slate-900 tracking-tightest">{t('free.legalNoticePage.title')}</h1>
      ...
      <div 
        ...
        dangerouslySetInnerHTML={t_html('free.legalNoticePage.content')}
      />
    </div>
  );
};
